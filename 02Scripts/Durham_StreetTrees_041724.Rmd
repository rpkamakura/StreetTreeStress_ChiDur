---
title: "Durham Modeling: Probit - R data"
author: "Renata"
date: "2022-12-08"
...
##if that still doesn't work, group P21 and Q21 into "downtown" and retry
# TreeLvlCombined_mod$SegmentMod <- as.character(TreeLvlCombined_mod$SegmentMod)
# TreeLvlCombined_mod$SegmentMod[TreeLvlCombined_mod$SegmentMod %in% c("P21", "Q21")] <- "Downtown"
# TreeLvlCombined_mod$SegmentMod <- as.factor(TreeLvlCombined_mod$SegmentMod)
# xdataCom <- TreeLvlCombined_mod[,c("TreeName", "SegmentMod", "Year", "BasalArea", "GenusGroup","CorrMulch", "PercentImperv","Powerline", "SiteType", "Gardenscape","PrunedCorrect", "CrownLight", "UrbanApp", "LandUse")]
# xdataCom$BasalArea <- scale(xdataCom$BasalArea)
# xdataCom <- xdataCom[!is.na(xdataCom$Gardenscape),]
...
```
Okay, so after that whole mess, the best option (lowest DIC) that gets your VIF values down for everything except segment is the model without crown light, percent impervious, and correct mulch
...
#write.csv(outMcomplexST6$parameters$beta, "../../01Data/03DurhamSampling/DurhamStreetTrees_CS_Beta_022724.csv")
...
From sensitivity, try without trunk damage and pruned correctly
...
Do a quick look at crapemyrtle just as a within-species example

```{r LAGE}

LAGEData <- TreeLvlCombined_mod[TreeLvlCombined_mod$Genus == "Lagerstroemia",]

###Set up for gjam run
TypeNames <- c("OC", "OC", "OC")

#get ydata
ydata <- LAGEData[,c("Defoliation", "Discoloration", "Dieback")]
...
#get x data
xdataCom <- LAGEData[,c("TreeName", "Segment", "Year", "BasalArea", "GenusGroup","CorrMulch", "PercentImperv",   "TreeSize", "Powerline", "SiteType", "Gardenscape","PrunedCorrect", "CrownLight", "UrbanApp", "LandUse")]
...
##might have to get rid of site type and land use?
LAGEData$SiteType <- as.character(LAGEData$SiteType)
LAGEData$SiteType[LAGEData$SiteType == "Other" | LAGEData$SiteType == "Yard"] <- "Other"
LAGEData$SiteType[LAGEData$SiteType == "01_SC" | LAGEData$SiteType == "SP"] <- "01_ScSp"
LAGEData$SiteType <- as.factor(LAGEData$SiteType)

LAGEData$LandUse <- as.character(LAGEData$LandUse)
LAGEData$LandUse[LAGEData$LandUse == "INST"] <- "Other"
LAGEData$LandUse <- as.factor(LAGEData$LandUse)


fmcomplex3 = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape + PrunedCorrect + SiteType + LandUse

out3var3 <- gjam(fmcomplex3, xdataCom, ydata, modelList = mlsimp)

##site type VIF still too high
fmcomplex4 = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape + PrunedCorrect + LandUse

out3var4 <- gjam(fmcomplex4, xdataCom, ydata, modelList = mlsimp)

##the whole model is trash to be honest, the sensitivities are all the same and the parameter estimates are all basically 0
