...
##2023 data
S23Campus <- read.csv(file.path(basicFilepath, "Duke2023_TreeHealthData_GrowingCond.csv"))
S23Street <- read.csv(file.path(basicFilepath, "Durham2023_TreeHealthData_GrowingCond.csv"))
S23Park   <- read.csv(file.path(basicFilepath, "Park2023_TreeHealthData_GrowingCond.csv"))


##2022 data
S22MainSheet <- read.csv(file.path(basicFilepath, "Durham2022_GrowingCond.csv"))
S22HTHC      <- read.csv(file.path(basicFilepath, "Durham2022_TreeHealthData.csv"))
...
#get the number of rows for storage df
...
S23Data$TearBarkPrune <- c(S23Street$Prune...tear.bark, S23Campus$Prune...tear.bark, S23Park$Prune...tear.bark)
...
h1[1] <- 1
h2 <- c(1:21, NA)
HTHC_dict <- data.frame(Original = h1, Simplified = h2)
...
trees <- intersect(tHTHC, tMain)
toCheck <- setdiff(tHTHC, tMain)
...
for (t in seq_along(trees)) {
  ...
  tdatHTHC <- S22HTHC[S22HTHC$NAME == trNm & !is.na(S22HTHC$CROWN_VIGOR), ]
  tdatMainS <- S22MainSheet[S22MainSheet$Tree.Name == trNm, ]
  tdat23 <- S23Data[S23Data$TreeName == trNm, ]
  
  nSample <- nrow(tdatHTHC)
  
  if (nSample == 0) {
    message("Error, no HTHC data for ", trNm, " (tree index: ", t, ")")
    next
  } else if (nSample == 1) {
    
    # Fill in 2022 data
    S22Data[stoInd, c("Date", "TreeName", "Segment", "Genus", "Species", "DBHTot", "LandUse", "SiteType",
                      "Street", "Sidewalk", "Powerline", "Mulch", "TreeGuard", "GatorBag", "Pruned",
                      "FungalConch", "BroDeadBranch", "TrunkDam", "Missing")] <-
      tdatMainS[, c("Date", "Tree.Name", "Street.Segment", "Genus", "Species", "Total.DBH", "Land.Use.Type",
                    "Site.type", "Street", "Sidewalk", "Powerline", "Mulch", "Protection.at.tree.base",
                    "Tree.diaper", "Fungal.Conch", "Broken.Branches", "TrunkDamage", "Removed")]
    
    S22Data[stoInd, c("Discoloration", "Defoliation", "Dieback", "CrownVigor", "Transparency", "CrownLightExposure")] <-
      tdatHTHC[, c("LEAF_DISCOLORATIONF", "LEAF_DEFOLIATIONF", "FINE_TWIG_DIEBACKF", "CROWN_VIGOR",
                   "CROWN_DENSITY_TRANS", "CROWN_LIGHT_EXPOSURE")]
    
    # Simplify dieback value
    ...
    if (nrow(tdat23) == 0) {
      message("No 2023 data for ", trNm, " (iteration: ", t, ")")
      trees2Check <- c(trees2Check, trNm)
    
      } else {
        S22Data[stoInd, c("LandUse", "SiteType", "PercImperv", "MulchVolcano", "HatrackPrune", "FlushCutPrune",
                        "TearBarkPrune", "Gardenscape", "VegDescription", "RootsVis", "RdSiLift", "RdSiCrack",
                        "LeafCurl")] <-
        tdat23[, c("LandUse", "SiteType", "PercImperv", "MulchVolcano", "HatrackPrune", "FlushCutPrune",
                   "TearBarkPrune", "Gardenscape", "VegDescription", "RootsVis", "RdSiLift", "RdSiCrack",
                   "LeafCurl")]
    }
    ...
    stoInd <- stoInd + 1
      ...
      message("More than one sample for tree ", trNm, " (tree number: ", t, ")")
      stop()
  }
}
...
AllTrees <- unique(c(S23Data$TreeName, S22Data$TreeName))
resampledTrees <- intersect(S23Data$TreeName, S22Data$TreeName)

# Create final combined dataset
years <- c(2022, 2023)
nRows <- length(AllTrees) * length(years) * 2  # Overestimate
TotalTreeMat <- data.frame(matrix(NA, nrow = nRows, ncol = length(S22DatCols)))
names(TotalTreeMat) <- S22DatCols

endNum22 <- nrow(S22Data)
endNum23 <- endNum22 + nrow(S23Data)

TotalTreeMat[1:endNum22, ] <- S22Data
TotalTreeMat[(endNum22 + 1):endNum23, ] <- S23Data
TotalTreeMat <- TotalTreeMat[!is.na(TotalTreeMat$TreeName), ]
TotalTreeMat$Year <- c(rep(2022, endNum22), rep(2023, nrow(S23Data)))

# Export combined dataset
write.csv(TotalTreeMat, "../01Data/Durham/DurhamAllYearsComb.csv", row.names = FALSE)
