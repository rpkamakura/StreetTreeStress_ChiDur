...
#install.packages( "C://Users/renat/Documents/Grad School/Dissertation Research/TreeHealthDurChi/02Scripts/ordReg_1.0.tar.gz", repos=NULL, type="source" )
#install.packages( "R:/clark/clark.unix/makeOrdReg/ordReg_1.0.tar.gz", repos=NULL, type="source" )
#install.packages( "C://Users/Ren/Documents/Graduate School/01 Dissertation Research/01 Research Folders/TreeHealthDurChi/02Scripts/ordReg_1.0.tar.gz", repos=NULL, type="source" )
#library('ordReg')
...
#TreeLvlCombined <- read.csv(paste(basicDataPath, "ProcessedData/TreeLevelDataCombined120722.csv", sep=""))

#read in full version of the tree dataset
##missing data
#TreeLvlCombined <- read.csv(paste(basicDataPath, "ProcessedData/TreeLevelDataCombined050123.csv", sep=""))
...
Had to go back and check trees that should be excluded and double-check why

## More groupings

Here we are going to mess around a bit with trying to do some more aggregation, because the data are still really noisy
...
Species groupings (had 15 groups)
* Acer platanoides (65)
* Gleditsia triacanthos (127)
* Fraxinus (combined)
* Tilia spp
* Ulmus spp 
* Maple spp (may need to be split out though)
- saccharinum/rubrum/x freemanii (sect. Rubra)
- saccharum (Sect. Saccharodendron, in same next lvl group with rubrum, more closely related to griseum - pentaphylla)
- palmatum (2 - sect. palmata) by itself

* rest done by functional group, probably

Maple phylogeny source: https://onlinelibrary.wiley.com/doi/pdf/10.1111/jse.12535
...
preds <- c("TreeName", "Segment", "Year", "BasalArea", "SppGroup","Mulch", "PercentImperv",   "TreeSize", "Powerline", "UrbanApp", "SiteType", "Gardenscape","PrunedCorrect", "CrownLight")

#For simplicity
# TreeLvlCombined_mod$TreeSize <- as.character(TreeLvlCombined_mod$TreeSize)
# TreeLvlCombined_mod$TreeSize[TreeLvlCombined_mod$TreeSize == "Small/Medium"] <- 0
# TreeLvlCombined_mod$TreeSize[TreeLvlCombined_mod$TreeSize == "Large"] <- 1
...
#so maybe avoid tree size and urban rec together? see which one makes more sense?

#ordinal
ordVars <- c("PercentImperv","Gardenscape","CrownLight")
...
#none of these seem particularly strong, should be fine with the ordinal variables
...
#segment and site type are high (0.56), and segment and mulch is borderline (0.4) but a lot of the rest are somewhat lower

#############################Visuals for the ones we want to interact
###Tree size and powerline
TS_pow <- table(TreeLvlCombined_mod[c("TreeSize", "Powerline")])
...
####################SpeciesGroups
OSppGroups <-TreeLvlCombined_mod[TreeLvlCombined_mod$SppGroup == "Other",]
totTrees <- length(OSppGroups$X)

##SppGroup x UrbanApp
PercSG_UA <- table(OSppGroups$UrbanApp)/totTrees
print(PercSG_UA)

##SppGroup x TreeSize
# PercSG_TS <- table(OSppGroups$TreeSize)/totTrees
# print(PercSG_TS)
...
ggplot(TreeLvlCombined_mod, aes(y = BasalArea, x = Mulch, colour = Mulch)) +
  geom_boxplot() +
  geom_jitter()
...
* Segment is related to a lot of things
  - species group 0.273 which is not horrible
  - mulch is 0.4281, which is not great
  - site type is 0.7156.... not good
* Might still be able to use tree size? 
...
#xdataCom <- TreeLvlCombined_mod[,c("TreeName", "Segment", "Year", "BasalArea", "SppGroup", "PercentImperv")]
...
Had to remove percent impervious and crown light because of VIF

Now we can go in and remove variables because of sensitivity

From model 4, the ones to try removing first are
* corrmulch
* pruned correct
* powerline
...
# write.csv(out3var12$parameters$betaTable, "../../05Outputs/ModelRuns041924/Chicago_3Ds_bestModel_12_Betas.csv")
# saveRDS(out3var12, "../../05Outputs/ModelRuns041924/Chicago_3Ds_bestModel_12.rds")

# ppars <- list(SAVEPLOTS=T)
# gjamPlot(out3var12, plotPars=ppars)
...
#Look at the variables
preds <- c("TreeName", "Segment", "Year", "BasalArea", "SppGroup","Mulch", "PercImperv",   "TreeSize", "Powerline", "UrbanApp", "SiteType", "Gardenscape","PrunedCorrect", "CrownLight")

#For simplicity
# summaryDat$TreeSize <- as.character(summaryDat$TreeSize)
# summaryDat$TreeSize[summaryDat$TreeSize == "Small/Medium"] <- 0
# summaryDat$TreeSize[summaryDat$TreeSize == "Large"] <- 1

summaryDat$CorrMulch <- summaryDat$Mulch
summaryDat$CorrMulch[summaryDat$MulchVolcano == 1] <- 0

#binary
binVars <- c("Powerline", "CorrPrune", "CorrMulch")
oVars <- binVars #so you can do pairwise comparisons
...
#ordinal
ordVars <- c("PercImperv","Gardenscape","CrownLight")
oVars_o <- ordVars #so you can do pairwise comparisons
...
for(o in 1:length(ordVars)){
  ...
  for (p in 1:length(oVars_o)){
    ...
    pol <- polychor(summaryDat[,c(o1)],summaryDat[,c(o2)])
    print(paste("The polychoric correlation for", o1, "and", o2, "is:", pol))
  }
...
#nominal
nomVars <- c("Segment", "SppGroup","Mulch","SiteType", "LandUse")
oVars_n <- nomVars #so you can do pairwise comparisons
...
looking at patterns of stress just for one species


```{r GLTR analyses}

GLTRData <- TreeLvlCombined_mod[TreeLvlCombined_mod$Genus == "Gleditsia",]
...
#get ydata
ydata <- GLTRData[,c("Defoliation", "Discoloration", "Dieback")]

#get the model list
mlsimp <- list(ng = 20000, burnin = 5000, typeNames = TypeNames, random="TreeName")

#function
fmcomplex = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape + PrunedCorrect + CrownLight + SiteType + LandUse

#get x data
xdataCom <- GLTRData[,c("TreeName", "Segment", "Year", "BasalArea", "GenusGroup","CorrMulch", "PercentImperv",   "TreeSize", "Powerline", "SiteType", "Gardenscape","PrunedCorrect", "CrownLight", "UrbanApp", "LandUse")]
#xdataCom <- TreeLvlCombined_mod[,c("TreeName", "Segment", "Year", "BasalArea", "SppGroup", "PercentImperv")]
xdataCom$BasalArea <- scale(xdataCom$BasalArea)
...
#####get rid of crown light
fmcomplex2 = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape + PrunedCorrect + SiteType + LandUse

out3var2 <- gjam(fmcomplex2, xdataCom, ydata, modelList = mlsimp)

##VIF still too high for site type
fmcomplex3 = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape + PrunedCorrect + LandUse

out3var3 <- gjam(fmcomplex3, xdataCom, ydata, modelList = mlsimp)


#try without mulch
fmcomplex4 = ~ Year + BasalArea + Powerline + Gardenscape + PrunedCorrect + LandUse

out3var4 <- gjam(fmcomplex4, xdataCom, ydata, modelList = mlsimp)

# try without pruning
fmcomplex5 = ~ Year + BasalArea + Powerline + CorrMulch + Gardenscape+ LandUse

out3var5 <- gjam(fmcomplex5, xdataCom, ydata, modelList = mlsimp)

#try without mulch or pruning
fmcomplex6 = ~ Year + BasalArea + Powerline + Gardenscape + LandUse
